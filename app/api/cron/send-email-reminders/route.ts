// app/api/cron/send-email-reminders/route.ts
...
function parseBody(raw: unknown): Body {
  ...
  return { days: typeof obj.days === "number" ? obj.days : undefined };
}
...
export async function POST(req: Request) {
  ...
  for (const s of subs) {
    if (!s.user?.id || !s.nextDate) continue;
    const entry =
      byUser.get(s.user.id) ??
      byUser.set(s.user.id, { email: s.user.email ?? null, name: s.user.name ?? null, items: [] })
        .get(s.user.id)!;
    entry.items.push({
      service: s.service,
      plan: s.plan ?? null,
  ...
  for (const [userId, row] of byUser.entries()) {
    if (!row.email) {
      skipped++;
      continue;
    }
      ...
      await resend.emails.send({
        ...
        subject:
          row.items.length === 1
            ...
            : `Upcoming ${row.items.length} subscription renewals`,
        html: renderHtml(row.name, row.items),
      });
      sent++;
    } catch (err) {
