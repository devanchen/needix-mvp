...
// Build providers array, then conditionally push Google if envs exist
const providers: any[] = [
...
// Google OAuth (Gmail read-only) — only added if env vars exist
if (process.env.GOOGLE_CLIENT_ID && process.env.GOOGLE_CLIENT_SECRET) {
  providers.push(
    Google({
      clientId: process.env.GOOGLE_CLIENT_ID!,
      clientSecret: process.env.GOOGLE_CLIENT_SECRET!,
      // request refresh token + Gmail read-only scope
      authorization: {
        params: {
          access_type: "offline",
          prompt: "consent",
